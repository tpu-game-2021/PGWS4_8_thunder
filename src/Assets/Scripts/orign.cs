...
public class orign : MonoBehaviour
{
    private Mesh mesh;
    float resetTime;
    ...
    void Update()
    {
        ...
        if (resetTime < 0.0f)
        {
            resetTime = Random.Range(1.0f, 3.0f);//—‹‚ª—Ž‚¿‚éŽžŠÔŠÔŠu
            generateThunder();
        }
    }
    ...
    {
        //
        Vector3 pos = new Vector3(0.0f, 500.0f, 0.0f);

        //
        Vector3 dir = new Vector3(Random.Range(-1.0f, 1.0f), Random.Range(-1.0f, 1.0f), 0.0f);
        dir.Normalize();
